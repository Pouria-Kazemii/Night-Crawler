    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-lg p-6">
                <form method="POST" action="{{ route('crawl-nodes.store') }}" class="rtl text-right space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select id="protocol" name="protocol"
                            ...
                            @foreach (['HTTP', 'HTTPS', 'SOCKS5'] as $protocol)
                                <option value="{{ $protocol }}" @selected(old('protocol') === $protocol)>{{ $protocol }}</option>
                            @endforeach
                        </select>
                        @error('protocol')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select id="is_verified" name="is_verified"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500">
                            <option value="1" @selected(old('is_verified') == '1')>بله</option>
                            <option value="0" @selected(old('is_verified') == '0')>خیر</option>
                        </select>
                        @error('is_verified')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
